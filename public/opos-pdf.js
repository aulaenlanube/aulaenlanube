...
if (window.OposDownloads) return;
    ...
    // Logo en runtime (misma imagen del repo; sin Blade aqui).
    let LOGO_BYTES = null, LOGO_W_PX = 0, LOGO_H_PX = 0;
    const LOGO_DISPLAY_HEIGHT_PT = 28;
    let LOGO_DISPLAY_WIDTH_PT = 40.63;
    let _logoP = null;
    function ensureLogo() {
        if (_logoP) return _logoP;
        _logoP = (async function () {
            try {
                const r = await fetch('/img/logo-documentos.jpg');
                if (!r.ok) return;
                const buf = await r.arrayBuffer();
                LOGO_BYTES = new Uint8Array(buf);
                const bmp = await createImageBitmap(new Blob([buf], { type: 'image/jpeg' }));
                LOGO_W_PX = bmp.width; LOGO_H_PX = bmp.height;
                LOGO_DISPLAY_WIDTH_PT = +(28 * (LOGO_W_PX / LOGO_H_PX)).toFixed(2);
            } catch (e) { /* sin logo: cae al vector de respaldo */ }
        })();
        return _logoP;
    }
    ...
    const margin = 50;                     // antes 56 — más ancho útil
    const contentWidth = pageWidth - 2 * margin;

    // Banda de cabecera y pie
    const headerLineY = pageHeight - 60;
    const footerLineY = 64;
    const contentTop = headerLineY - 22;
    const contentBottom = footerLineY + 18;
...
const P = BRAND.pdf;
    ...
    const buildHeaderStream = function (chrome) {
        const { hasLogoPdf, headerTitle, headerSubtitle } = chrome;
        let s = '';
        if (hasLogoPdf) {
            // `cm` modifica la CTM y afectaría al texto siguiente
            // si no la aislamos: la imagen va en su propio q...Q.
            // Se dibuja en una caja LOGO_W × LOGO_H pt con esquina
            // inferior izquierda en (margin, pageHeight - logoBottom).
            const lw = LOGO_DISPLAY_WIDTH_PT;
            const lh = LOGO_DISPLAY_HEIGHT_PT;
            const lx = margin;
            const ly = pageHeight - 22 - lh; // 22pt desde el borde superior
            s += 'q ' + F(lw) + ' 0 0 ' + F(lh) + ' ' + F(lx) + ' ' + F(ly) + ' cm /Im0 Do Q\n';
        } else {
            // Fallback vectorial: capucha + tabla + botón + wordmark
            const logoCx = margin + 13;
            const logoCy = pageHeight - 38;
            s += 'q\n';
            s += buildLogoVector(logoCx, logoCy);
            s += C3(P.primary) + ' rg\n';
            s += 'BT /F2 14 Tf ' + F(margin + 32) + ' ' + F(pageHeight - 41) + ' Td (OposicionesIA) Tj ET\n';
            s += 'Q\n';
        }
        // Título contextual y línea: en bloque q...Q propio para
        // garantizar que la CTM del logo no se filtra.
        s += 'q\n';
        if (headerTitle && headerSubtitle) {
            // Dos líneas a la derecha: cuerpo (negrita) + especialidad (debajo).
            s += C3(P.muted) + ' rg\n';
            const w1 = approxWidth(headerTitle, 10.5, true);
            s += 'BT /F2 10.5 Tf ' + F(pageWidth - margin - w1) + ' ' + F(pageHeight - 36) + ' Td (' + pdfEsc(headerTitle) + ') Tj ET\n';
            const w2 = approxWidth(headerSubtitle, 9.5, false);
            s += 'BT /F1 9.5 Tf ' + F(pageWidth - margin - w2) + ' ' + F(pageHeight - 49) + ' Td (' + pdfEsc(headerSubtitle) + ') Tj ET\n';
        } else if (headerTitle) {
            s += C3(P.muted) + ' rg\n';
            const tW = approxWidth(headerTitle, 12, true); // /F2 = bold
            const tX = pageWidth - margin - tW;
            const tY = pageHeight - 41;
            s += 'BT /F2 12 Tf ' + F(tX) + ' ' + F(tY) + ' Td (' + pdfEsc(headerTitle) + ') Tj ET\n';
        }
        s += C3(P.line) + ' RG 0.6 w\n';
        s += margin + ' ' + headerLineY + ' m ' + (pageWidth - margin) + ' ' + headerLineY + ' l S\n';
        s += 'Q\n';
        return s;
    };
    ...
    const buildFooterStream = function (chrome, pageNum, totalPages) {
        ...
        let s = 'q\n';
        // Línea sobre footer
        s += P.line[0].toFixed(3) + ' ' + P.line[1].toFixed(3) + ' ' + P.line[2].toFixed(3) + ' RG 0.6 w\n';
        s += margin + ' ' + footerLineY + ' m ' + (pageWidth - margin) + ' ' + footerLineY + ' l S\n';
        s += P.muted[0].toFixed(3) + ' ' + P.muted[1].toFixed(3) + ' ' + P.muted[2].toFixed(3) + ' rg\n';
        const footerY = footerLineY - 14;
        // Page X de Y derecha (localizado); se mide primero para saber
        // cuánto sitio queda a la izquierda para el título.
        const pageStr = FL.page + ' ' + pageNum + ' ' + FL.sep + ' ' + totalPages;
        const pageW = approxWidth(pageStr, 9);
        const pageX = pageWidth - margin - pageW;
        // Marcado IA (Reglamento (UE) 2024/1689, art. 50): mención visible
        // "Generado con IA" centrada en el pie (7pt, mismo gris muted).
        const aiText = 'Generado con IA';
        const aiW = approxWidth(aiText, 7);
        const aiX = pageWidth / 2 - aiW / 2;
        // Izquierda: título del tema si se ha indicado (footerTitle);
        // si no, la URL de marca de siempre. Se trunca con «…» si no
        // cabe sin invadir la mención centrada ni el número de página.
        const maxLeftW = Math.min(pageX - margin - 14, aiX - margin - 10);
        let leftText = String(footerTitle || '').trim() || 'oposicionesia.com';
        if (footerTitle && approxWidth(leftText, 9) > maxLeftW) {
            while (leftText.length > 1 && approxWidth(leftText + '…', 9) > maxLeftW) leftText = leftText.slice(0, -1);
            leftText = leftText.trim() + '…';
        }
        s += 'BT /F1 9 Tf ' + margin + ' ' + footerY + ' Td (' + pdfEsc(leftText) + ') Tj ET\n';
        s += 'BT /F1 7 Tf ' + aiX.toFixed(2) + ' ' + footerY + ' Td (' + pdfEsc(aiText) + ') Tj ET\n';
        s += 'BT /F1 9 Tf ' + pageX.toFixed(2) + ' ' + footerY + ' Td (' + pdfEsc(pageStr) + ') Tj ET\n';
        s += 'Q\n';
        return s;
    };

    // ── Marca de agua ──
    // "OposicionesIA.com" en grande, horizontal, centrado y muy transparente
    // (vía /GSwm). Mismos colores del logo de la web: "OposicionesIA" en azul
    // de marca y ".com" en ámbar. Ocupa casi todo el ancho útil.
    const buildWatermark = function () {
        const t1 = 'OposicionesIA', t2 = '.com';
        const target = contentWidth * 0.96;
        const natural = helvWidth(t1 + t2, 100, true);   // ancho medido a 100pt
        const size = 100 * target / natural;             // escala para ~96% del ancho útil
        const w1 = helvWidth(t1, size, true);
        const w2 = helvWidth(t2, size, true);
        const startX = (pageWidth - (w1 + w2)) / 2;
        const wy = pageHeight / 2 - size * 0.34;          // centrado vertical aproximado
        let s = 'q /GSwm gs\n';
        s += C3(P.primary) + ' rg BT /F2 ' + size.toFixed(2) + ' Tf ' + startX.toFixed(2) + ' ' + wy.toFixed(2) + ' Td (' + pdfEsc(t1) + ') Tj ET\n';
        s += C3(P.accent)  + ' rg BT /F2 ' + size.toFixed(2) + ' Tf ' + (startX + w1).toFixed(2) + ' ' + wy.toFixed(2) + ' Td (' + pdfEsc(t2) + ') Tj ET\n';
        s += 'Q\n';
        return s;
    };
    ...
    async function exportPdf(content, filename, onToast, headerTitle, lang, sections, headerSubtitle, footerTitle) {
        ...
        await ensureLogo(); const logoBytes = LOGO_BYTES;
        const hasLogoPdf = !!(logoBytes && logoBytes.byteLength > 0 && LOGO_W_PX > 0 && LOGO_H_PX > 0);
        // Lo que la cabecera y el pie necesitan saber de ESTE documento.
        const chrome = { hasLogoPdf, headerTitle, headerSubtitle, footerTitle, FL };
        ...
        if (hasLogoPdf) addImageObj('Im0', logoBytes, LOGO_W_PX, LOGO_H_PX);
        ...
        const extGStateResource = ' /ExtGState << /GSwm 8 0 R >>';
        ...
        for (let p = 0; p < pages.length; p++) {
            ...
            stream += buildHeaderStream(chrome);
