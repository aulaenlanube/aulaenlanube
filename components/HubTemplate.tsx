...
const proseCls =
  "text-[15px] leading-7 text-zinc-700 dark:text-zinc-300 [&_a]:text-blue-600 dark:[&_a]:text-blue-400 [&_a]:underline [&_b]:font-semibold [&_strong]:font-semibold " +
    ...
    <div className={`mt-6 grid grid-cols-1 gap-5 ${cols}`}>
      {block.items.map((c) => (
        <Link
          key={c.path}
          href={c.path}
          className="group block overflow-hidden rounded-xl ring-1 ring-zinc-200 dark:ring-white/10 transition hover:-translate-y-0.5 hover:shadow-md hover:ring-blue-300 dark:hover:ring-blue-500/40"
        >
          {c.image ? (
...
function AlertBox({ block }: { block: Extract<HubBlock, { t: "alert" }> }) {
  ...
  return (
    ...
    >
      {block.title && (
        <p className={`font-bold ${danger ? "text-red-700 dark:text-red-300" : "text-blue-700 dark:text-blue-300"}`}>{block.title}</p>
      )}
      {block.desc && (
        <p className={`mt-1 text-sm leading-6 ${danger ? "text-red-600 dark:text-red-400 dark:text-red-300" : "text-blue-700 dark:text-blue-300"}`}>{block.desc}</p>
      )}
    </div>
  );
}
...
function Block({ block }: { block: HubBlock }) {
  switch (block.t) {
    ...
    case "heading":
      return <h2 className="mt-12 text-2xl font-bold text-slate-700 dark:text-slate-200 sm:text-3xl">{block.text}</h2>;
    case "html":
      return block.boxed ? (
        <div className="mt-6 rounded-xl border border-sky-200 dark:border-sky-500/30 bg-sky-50/60 dark:bg-sky-500/10 px-6 py-5">
    ...
    case "slides":
      return (
        <div className="mt-6 aspect-video w-full overflow-hidden rounded-xl ring-1 ring-zinc-200 dark:ring-white/10">
          <iframe
            src={block.url}
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:py-10">
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_19rem] lg:gap-10">
        <article className="min-w-0">
          <h1 className="flex items-baseline gap-2 text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100 sm:text-3xl">
            {hadTriangle && <span className="text-blue-500 dark:text-blue-400">▷</span>}
