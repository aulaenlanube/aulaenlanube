...
// ════════════════════════════════════════════════════════════════════════════
// 4 · La pregunta del vocal (dos ramas)
// Rejilla: raíz centrada arriba; dos cajas de 308 en x = 16 y x = 356.
// ════════════════════════════════════════════════════════════════════════════

const W_RAMA = 308;
const X_RAMA_A = 16;
const X_RAMA_B = MEDIO - 16 - W_RAMA; // 356
const Y_RAMA = 152;

const SI: string[] = [
  "Explicas qué hace ese trozo y por qué está ahí.",
  "Sabes qué cambia en la salida si tocan ese valor.",
  "Enseñas un fallo que detectaste tú y cómo lo corregiste.",
  "Dices sin rodeos que usaste un agente: es una herramienta.",
];

const NO: string[] = [
  "Vino una librería que no sabes ni cómo se llama.",
  "Cambian un parámetro y no sabes qué va a salir.",
  "El diseño es impecable y la lógica está vacía.",
  "Lo escondes, y eso el tribunal sí lo huele.",
];

export function PreguntaDelVocal() {
  const lineasSi = SI.map((s) => parteEnLineas(`• ${s}`, 34)).flat();
  const lineasNo = NO.map((s) => parteEnLineas(`• ${s}`, 34)).flat();
  const h = altoCaja(Math.max(lineasSi.length, lineasNo.length));
  const yPie = Y_RAMA + h + 16;
  const alto = yPie + 34 + 12;
      ...
      pasos={[
        {
          id: "vocal",
          tono: "gris",
          titulo: "«Y si aquí cambiamos esto, ¿qué pasa?»",
          texto:
            "Es la pregunta que rompe las defensas, y llega siempre. No pregunta si usaste IA: pregunta si entiendes lo que traes. La respuesta honesta a ambas ramas es defendible; lo que no se puede defender es no saber cuál es la tuya.",
          clave: "Si no estás dispuesto a que toquen una línea, no la lleves.",
        },
        {
          id: "si",
          tono: "verde",
          titulo: "Sabes qué pasa · aquí se gana",
          texto:
            "Que lo haya escrito un agente no resta nada: en cualquier empresa de software se trabaja así. Lo que puntúa es dirigir la herramienta, verificar el resultado y entender cada decisión. Enseñar la conversación con el agente como evidencia del método suma; esconderla, resta.",
          clave: "Lleva poco código y entiéndelo entero. Sesenta líneas que dominas valen más que doscientas que te vinieron dadas.",
        },
        {
          id: "no",
          tono: "rosa",
          titulo: "No lo sabes · aquí se acaba",
          texto:
            "No se cae por usar IA: se cae por llevar algo que no se entiende. Si de doscientas líneas usas sesenta, las otras ciento cuarenta son ciento cuarenta preguntas que no puedes responder. Recorta todo lo que no sepas explicar en una frase.",
          clave: "La prueba de diez minutos, en casa: tapa el código y cuenta qué hace. Después destápalo y explica cada bloque.",
        },
      ]}
      etiqueta="Dos ramas a partir de la pregunta del vocal: si sabes qué pasa al cambiar una línea, la defensa se gana; si no lo sabes, se pierde."
      pie="La misma pregunta, dos finales. Lo que decide no es haber usado IA: es entender lo que llevas. Pulsa cada rama."
      dibuja={(ctx) => (
        <>
          <Zona ctx={ctx} paso="vocal" etiqueta="Ver la pregunta del vocal">
            <rect
              x={90}
              y={22}
              width={MEDIO - 180}
              height={58}
              rx={29}
              fill={ctx.sel === "vocal" ? TONOS.gris.suave : "#ffffff"}
            ...
            <Rotulo
              x={MEDIO / 2}
              y={46}
              texto="El vocal te cambia una línea:"
              tam={FS}
              peso={500}
              color={TENUE}
              ancla="middle"
            ...
            <Rotulo
              x={MEDIO / 2}
              y={68}
              texto="«y si aquí ponemos esto, ¿qué pasa?»"
              tam={FS_TIT}
              peso={800}
              color={TINTA}
              ancla="middle"
          ...
          <Curva
            d={`M ${MEDIO / 2} 84 C ${MEDIO / 2} 118, ${X_RAMA_A + W_RAMA / 2} 112, ${X_RAMA_A + W_RAMA / 2} ${Y_RAMA - 6}`}
            tono="verde"
            uid={ctx.uid}
            grosor={3}
          />
          <Curva
            d={`M ${MEDIO / 2} 84 C ${MEDIO / 2} 118, ${X_RAMA_B + W_RAMA / 2} 112, ${X_RAMA_B + W_RAMA / 2} ${Y_RAMA - 6}`}
            tono="rosa"
            uid={ctx.uid}
            grosor={3}
          />
          <Rotulo x={X_RAMA_A + W_RAMA / 2 - 6} y={128} texto="lo sé" tam={FS} peso={700} color={TONOS.verde.fuerte} ancla="end" />
          <Rotulo x={X_RAMA_B + W_RAMA / 2 + 6} y={128} texto="no lo sé" tam={FS} peso={700} color={TONOS.rosa.fuerte} />
          ...
          <Caja
            ...
            titulo="Se gana aquí"
            lineas={lineasSi}
            etiqueta="Ver: qué pasa si sabes responder"
          />
          <Caja
            x={X_RAMA_B}
            y={Y_RAMA}
            ...
            ctx={ctx}
            paso="no"
            titulo="Se acaba aquí"
            lineas={lineasNo}
            etiqueta="Ver: qué pasa si no sabes responder"
          />

          <Pildora
            x={(MEDIO - anchoPildora("Si no dejarías que tocaran una línea, no la lleves")) / 2}
            y={yPie}
            texto="Si no dejarías que tocaran una línea, no la lleves"
            tono="ambar"
            alto={34}
          />
        </>
